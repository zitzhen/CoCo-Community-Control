...
const types = {
  ...
  methods: [
    // 获取字符串的前几个字符
    {
      ...
      valueType: 'string',
      tooltip: '获取字符串的前几个字符',
      blockOptions: { callMethodLabel: false }  // 添加这一行
    },
    // 删除分隔符前的内容
    {
...
class TextProcessingTool extends InvisibleWidget {
  ...
  // 获取字符串的前几个字符（来自TEST_1_0_WIDGET）
  getSubstring(text, length) {
    if (typeof text !== 'string') {
      throw new Error('输入的字符串必须是字符串类型');
    }
    if (typeof length !== 'number' || length < 0) {
      throw new Error('获取的字符数必须是非负整数');
    }
